...
### From 2026-07-18 U9 review

Multi-lens review of the U9 filter-list management + persistence unit (commit `5351244`). The plan's three security follow-ups all landed and verified correct: HTTPS-only downloads (URL + redirect targets, in both `AdBlockListStore::startDownload` and `AdBlockListDownloader`), SHA-256 integrity pinning in `handleDownload`, and the most-restrictive scriptlet permission mask on every compiled list. Two findings were fixed in-branch (same commit):

- **List-text reads ran on the NetworkProcess main run loop** (P2, performance) — `rebuildEngine()` read every enabled list's text with a blocking `FileSystem::readEntireFile` on the main thread before handing the bytes to the off-queue compile, so a multi-MB EasyList read blocked all NetworkProcess loads on every toggle/custom-rule edit/download completion — contradicting the U9 approach's "rebuild off the main queue." Fixed by replacing `setEngineFromLists(Vector<ListInput>)` with `setEngineFromListFiles(Vector<ListFileInput>, inlineLists, …)`: the store gathers only file *paths* + the small custom-rules blob on the main thread, and the manager reads the texts on its WorkQueue (the same off-main queue it already uses for `.dat` cache I/O), keeping read → compile → cache-save serialized on one queue.
- **Persisted `filename` used unsanitized for read and delete** (P3, security, defense-in-depth) — `filename` is generated internally as `<sha256-hex>.txt` but read back verbatim from `adblock-config.json` and joined onto `lists/` for `readEntireFile` (rebuild) and `deleteFile` (remove); a tampered config (`"filename": "../../…"`) could traverse out of `lists/`. Fixed with an `isSafeListFilename` guard at the `loadConfig` trust boundary (rejects `.`/`..`/`/`/`\`), clearing an unsafe value to empty — the existing "not-yet-downloaded" state that `rebuildEngine`/`removeSubscription` already skip, so it can neither be read nor deleted and is re-downloaded to a safe name.
...
### From 2026-07-20 restart-persistence fix (U9/U10)

While re-adding the deferred U9 persistence tests as a Swift Testing suite at U10 (`AdBlockListManagementTests.swift`), the three NetworkProcess-restart cases — `customRulesSurviveRestart`, `warmDatSkipsReparseOnRestart`, `deletedDatRebuildsFromListText` — failed: after a `_terminateNetworkProcess()` relaunch, blocking never resumed. One finding was fixed in-branch; it exposes an architectural divergence from Brave worth reconsidering.

- **Fixed in-branch — a config-less session clobbered the process-global engine on relaunch** (correctness, confidence 100)

  The `AdBlockManager` is process-global (KTD3: one compiled engine + one enable gate per NetworkProcess, shared across all data stores). `AdBlockListStore::load()` unconditionally re-asserted that shared state from *its own* session's config — `setEnabled(m_enabled)` then rebuild/warm-load the engine. On relaunch, the framework's **default** data store session (which had never configured adblock — no config file, `enabled=0`, no lists) ran `load()` right after the configured test session and rebuilt an *empty* engine with the gate off, wiping the engine the configured session had just restored. It only manifested after restart because on first launch the configured session happened to be the last writer. Fix: `load()` now returns early without touching the shared manager when this session has no persisted config file on disk; every runtime mutation persists config before rebuilding, so a genuinely-configured session is never skipped.

- **Point to reconsider — per-session config each driving one process-global engine is not Brave's model** (architecture, confidence 90)

  The fix is a targeted guard, not the architecture Brave uses, and it leaves a residual gap: if *two* persistent sessions each carried adblock config, they would still fight over the single shared engine (last writer on `load()` wins). Brave avoids this class of bug structurally. Its `AdBlockService` is likewise process-global (owned by `BraveBrowserProcessImpl`, not per-profile), but it is anchored to **one** config source and **one** storage root — constructed with the global `local_state()` prefs and the *initial* profile directory (`profile_manager()->GetInitialProfileDir()`), regardless of how many profiles exist — so there is never a second loader. Per-profile / per-site enablement is not an engine rebuild at all: it is a **request-time gate**. In `OnBeforeURLRequest_AdBlockTPPreWork`, `ctx->allow_brave_shields()` / `ctx->allow_ads()` (read from that profile's `HostContentSettingsMap`) short-circuit before the global engine is ever queried; the engine is compiled once from global list config and whether to *apply* a block is decided per-navigation from per-profile Shields settings. Brave-aligned direction if this needs to be closed properly: keep list/engine config in one process-global place (a single owner, not per-data-store), and express the per-session enable/allowlist distinction as a request-time gate — extending the per-host allowlist check already in `AdBlockManager::checkRequest`, which is the same shape as Brave's request-time Shields check — rather than as a per-session engine rebuild. Not required for the tests (which pass with the guard); flagged so the single-configured-store assumption baked into `AdBlockListStore` is a deliberate, revisited choice rather than an accident.
